...
test('다크 UI는 surface layering과 제한된 accent 토큰을 갖는다',async()=>{
  ...
  for(const token of ['--abyss:#06080a','--workspace:#0b0f13','--panel:#12171c','--panel-elevated:#181f26','--interactive:#202933','--accent:#c6ff4a','--cyan:#4cb8e8','--purple:#9a7bff']) assert.ok(css.includes(token),token);
  assert.match(css,/\.패널헤더\{[^}]*font-size:12px/);
  assert.match(css,/\.타임클립 span\{[^}]*font-size:10px/);
});
