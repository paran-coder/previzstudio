...
export const CAMERA_SAFETY_DEFAULTS = Object.freeze({
  minTargetDistance: 0.6,
  minHeight: 0.28,
  maxHeight: 60,
  maxCoordinate: 250,
});
...
export function cameraTargetDistance(position, target) {
  if (!isFiniteVec3(position) || !isFiniteVec3(target)) return 0;
  return Math.hypot(target[0]-position[0], target[1]-position[1], target[2]-position[2]);
}

export function validateCameraStateBasic(state, options={}) {
  const rules={...CAMERA_SAFETY_DEFAULTS,...options};
  const position=state?.position,target=state?.target;
  if(!isFiniteVec3(position)||!isFiniteVec3(target)||!Number.isFinite(state?.lens)){
    return {ok:false,level:'danger',code:'non_finite',message:'카메라 좌표에 유효하지 않은 숫자가 있습니다.'};
  }
  if(position.some(v=>Math.abs(v)>rules.maxCoordinate)||target.some(v=>Math.abs(v)>rules.maxCoordinate)){
    return {ok:false,level:'danger',code:'out_of_bounds',message:'카메라 좌표가 씬의 안전 범위를 벗어났습니다.'};
  }
  if(position[1]<rules.minHeight){
    return {ok:false,level:'danger',code:'below_ground',message:'카메라가 지면 아래 또는 너무 낮은 위치에 있습니다.'};
  }
  if(position[1]>rules.maxHeight){
    return {ok:false,level:'danger',code:'height_limit',message:'카메라 높이가 안전 범위를 벗어났습니다.'};
  }
  const distance=cameraTargetDistance(position,target);
  if(distance<rules.minTargetDistance){
    return {ok:false,level:'danger',code:'target_too_close',message:`카메라와 타겟 거리는 ${rules.minTargetDistance.toFixed(1)}m 이상이어야 합니다.`,distance};
  }
  return {ok:true,level:'safe',code:'ok',message:'안전 · 카메라 좌표 유효',distance};
}
