...
let undoStack=[],redoStack=[],gizmoHistoryBefore=null;const HISTORY_LIMIT=60;
...
function recordHistory(before,label='편집'){const after=sceneSnapshot();if(!before||sameScene(before,after))return;undoStack.push({before,after,label});if(undoStack.length>HISTORY_LIMIT)undoStack.shift();redoStack=[];syncHistoryUI();}
...
function mutateWithHistory(label,mutator,message=label){const before=sceneSnapshot();mutator();afterDocumentEdit('',false);recordHistory(before,label);if(message)showToast(message,1400);}
...
function endGizmoHistory(){if(!gizmoHistoryBefore)return;const before=gizmoHistoryBefore;gizmoHistoryBefore=null;recordHistory(before,editTarget==='camera'?'카메라 Gizmo':'배우 Gizmo');showToast('Transform을 수정했습니다.',1000);}
...
async function loadScene(doc){
  ...
  sceneDoc=doc;await engine.loadDocument(sceneDoc);engine.onTime=(time,playing)=>updateTimeUI(time,playing);engine.onDocumentEdit=syncFromEngineEdit;engine.onTransformStart=beginGizmoHistory;engine.onTransformEnd=endGizmoHistory;clearHistory();
