...
export async function handleRequest(req,res){if(req.method==='GET'&&req.url==='/api/health')return json(res,200,{ok:true,version:'1.3.3',director:'local-command-parser',render:'browser'});if(!['GET','HEAD'].includes(req.method))return json(res,405,{error:'지원하지 않는 메서드입니다.'});return serveStatic(req,res);}
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)){http.createServer((req,res)=>handleRequest(req,res)).listen(PORT,'127.0.0.1',()=>console.log(`Previz Studio v1.3.3: http://127.0.0.1:${PORT}`));}
